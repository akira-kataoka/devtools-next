...
## 🔗 URL クエリ統一パターン (Phase 233-295、10 種) — チーム共有用 URL リンク
...
**安全設計**: 破壊的な REST 呼び出し (POST/PATCH/DELETE) と Apex 実行は自動実行せず、ユーザー確認後に手動「送信/▶ 実行」が必要。URL リンクが意図せず実行されないようガードしています。「🔗 リンク」ボタンは **8 箇所** (search/SOQL/describe/inspector/metadata/design/rest/apex) に配置。
