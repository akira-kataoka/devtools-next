...
## 🛡️ PROD (本番組織) 誤操作防止 — 2 段階防御 × 6 経路カバー (Phase 288-293) + 3 モード ENV 統一 (Phase 117/288/333)

本番組織での「ボタン押下 → 即実データ変更」事故を物理的にブロックする多層防御を装備しています。

### 第 1 層: 視覚警告 (3 モード ENV 表示 100% 統一、Phase 117 / 288 / 333)

3 モードすべてで環境バッジを統一表示 — Phase 333 で popup にも追加して 100% 統一達成:
- ⚙️ **管理者 (popup)** — `popupEnvBadge` (Phase 333、ヘッダー左) Sandbox / PROD / Dev 表示、PROD はパルスアニメ
- 💻 **開発者 (panel / tool)** — `envBadge` (Phase 117) PROD pulse animation
- 👤 **ユーザー (mini-panel)** — `hdrEnv pill` (Phase 288、PROD は 2.4s ループパルス、`prefers-reduced-motion` 対応)
...
## 🗂️ クイック実行 templates グルーピング (Phase 313-318、6 系統 × 102 templates × 26 カテゴリ)
...
| 🔎 **SOQL クエリ** (panel) | panel + tool toolbar | **25** | 5 | 基本 / 開発 (Tooling) / 業務分析・棚卸し / ライセンス・権限 / セキュリティ・管理者 |
...
| 📝 **mini-panel SOQL** | mini-panel (Salesforce 画面上) | **10** | 3 | 基本 (営業/開発) / 業務 / 管理者・セキュリティ |
| **合計** | — | **102** | **26** | — |
...
- [🔗 URL クエリ統一パターン](#-url-クエリ統一パターン-phase-233-29812-種--チーム共有用-url-リンク) — 12 種 (admin/search/describe/inspector/soql/metadata/design/rest/apex/login/apiurl)
- [🛡️ PROD 誤操作防止](#-prod-本番組織-誤操作防止--2-段階防御--6-経路カバー-phase-288-293--3-モード-env-統一-phase-117288333) — 2 段階防御 × 6 経路カバー + 3 モード ENV 統一 (Phase 333 で 100% 達成)
- [🗂️ クイック実行 templates グルーピング](#-クイック実行-templates-グルーピング-phase-313-3186-系統--102-templates--26-カテゴリ) — 6 系統 × 102 templates × 26 カテゴリ
...
| 21 | ⭐ **組織全体スナップショット (orgSnapshot、Phase 240)** | 1 クリックで 7 章統合 (組織情報 + ライセンス + 主要オブジェクト統計 + アクティブユーザー + ストレージ + Apex 集計 + 設定変更履歴 直近 30 日) | 監査資料 1 発出力 / 引継ぎ書 / 組織健康診断レポート |
...
| 21 | ⭐ orgSnapshot | (入力不要) | 1 クリックで 7 章統合 (Phase 240) — 監査資料 / 引継ぎ書 / 組織健康診断レポート |
