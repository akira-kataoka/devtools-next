...
DevToolsNext には起動できる 3 つの入口があります。**初めての方は ① の popup 上部の「全画面で開く」ボタンが最もおすすめです**:

| # | 入口 | 起動方法 | 用途 | 特徴 |
|---|---|---|---|---|
| ① | **🛠️ 全画面タブ (推奨)** | Chrome 拡張アイコン → popup 上部の「🛠️ DevToolsNext を全画面で開く」ボタン (青い大型 CTA) | **メイン UI**。全機能をホームダッシュボード経由で利用 | フルスクリーンで作業可能、ホームのカードで機能を視覚的に選択 |
| ② | **拡張ポップアップ** | Chrome 拡張アイコンをクリック | クイックアクセス。**全画面起動 / セッション確認 / Setup 起動 / Login as User / ID 解析 / 便利リンク** | ライトな確認用途、リサイズ可能 (環境による) |
| ③ | **DevTools パネル** | Salesforce タブで F12 → 「Salesforce」タブ | F12 開発者向け。② と同じ全機能 | デバッグ時に Network/Console と並べて使える |

各入口とも **「🏠 ホーム」ダッシュボード** から始まり、主要 6 機能 (SOQL / Inspector / データエクスポート / Apex / 設計書 / Limits) と補助 7 機能のカードから直感的に選べます。
