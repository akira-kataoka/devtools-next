...
- [🎨 CSS デザイントークン体系](#-css-デザイントークン体系-v3900--phase-180-187-で導入) — 3 モード共通の 18 種トークン (sp-hair + sp-0..7 + sp-tag-y + sp-card-y / r-xs..xl + r-tag + r-pill)
...
3 モード (popup / panel+tool / mini-panel) で同じスペーシング・角丸を使うため、CSS カスタムプロパティ (`var(--*)`) として 18 種のデザイントークンを定義しています。`panel.css` と `popup.css` の `:root` ブロックに**完全同値**で定義され、ハードコード値の混在を防ぎます。

### スペーシング (4px ベース・8 段階 + セマンティック 3 段階) — `--sp-*`
