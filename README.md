...
- [🎨 CSS デザイントークン体系](#-css-デザイントークン体系-v3900--phase-180-184-で導入) — 3 モード共通の 15 種トークン (sp-0..7 / r-xs..xl + r-tag + r-pill)
...
## 🎨 CSS デザイントークン体系 (v3.90.0 〜) — Phase 180-184 で導入

3 モード (popup / panel+tool / mini-panel) で同じスペーシング・角丸を使うため、CSS カスタムプロパティ (`var(--*)`) として 15 種のデザイントークンを定義しています。`panel.css` と `popup.css` の `:root` ブロックに**完全同値**で定義され、ハードコード値の混在を防ぎます。
