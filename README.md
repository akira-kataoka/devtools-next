...
- [🎨 CSS デザイントークン体系](#-css-デザイントークン体系-v3900--phase-180-183-で導入) — 3 モード共通の 14 種トークン (sp-0..7 / r-xs..xl + r-pill)
...
## 🎨 CSS デザイントークン体系 (v3.90.0 〜) — Phase 180-183 で導入

3 モード (popup / panel+tool / mini-panel) で同じスペーシング・角丸を使うため、CSS カスタムプロパティ (`var(--*)`) として 14 種のデザイントークンを定義しています。`panel.css` と `popup.css` の `:root` ブロックに**完全同値**で定義され、ハードコード値の混在を防ぎます。

### スペーシング (4px ベース・8 段階) — `--sp-*`
...
- **スペーシング**: 1px / 3px / 5px / 7px / 14px / 18px / 20px / 32px → 個別判断 (例: home-card の `padding: 20px 18px` は visual hierarchy 上の意図的選択)
- **角丸**: 5px / 14px / `50%` → 50% は user-avatar 円形のため永続保持、5px/14px は将来統合候補
